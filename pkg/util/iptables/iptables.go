...
func newInternal(exec utilexec.Interface, protocol Protocol, lockfilePath14x, lockfilePath16x string) Interface {
	...
	if err != nil {
		klog.Errorf("NOK5G21-145: Error checking iptables version, assuming version at least %s: %v", MinCheckVersion, err)
...
func (runner *runner) SaveInto(table Table, buffer *bytes.Buffer) error {
	...
	klog.Errorf("NOK5G21-145: running %s %v", iptablesSaveCmd, args)
...
func (runner *runner) restoreInternal(args []string, data []byte, flush FlushFlag, counters RestoreCountersFlag) error {
	...
	if len(runner.restoreWaitFlag) == 0 {
		...
		defer func(locker iptablesLocker) {
			if err := locker.Close(); err != nil {
				klog.Errorf("NOK5G21-145: Failed to close iptables locks: %v", err)
			}
		}(locker)
	}
	...
	klog.Errorf("NOK5G21-145: running %s %v", iptablesRestoreCmd, fullArgs)
...
func (runner *runner) runContext(ctx context.Context, op operation, args []string) ([]byte, error) {
	...
	klog.Errorf("NOK5G21-145: running iptables: %s %v", iptablesCmd, fullArgs)
	if ctx == nil {
...
func (runner *runner) checkRuleWithoutCheck(table Table, chain Chain, args ...string) (bool, error) {
	iptablesSaveCmd := iptablesSaveCommand(runner.protocol)
	klog.Errorf("NOK5G21-145: running %s -t %s", iptablesSaveCmd, string(table))
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		klog.Errorf("NOK5G21-145: DBG: fields is not a superset of args: fields=%v  args=%v", fields, args)
	}
	...
	for {
		_ = utilwait.PollImmediateUntil(interval, func() (bool, error) {
			for _, table := range tables {
				if _, err := runner.EnsureChain(table, canary); err != nil {
					klog.Errorf("NOK5G21-145: Could not set up iptables canary %s/%s: %v", string(table), string(canary), err)
					return false, nil
				}
			}
			return true, nil
		...
		err := utilwait.PollUntil(interval, func() (bool, error) {
			if exists, err := runner.ChainExists(tables[0], canary); exists {
				...
				klog.Errorf("NOK5G21-145: Could not check for iptables canary %s/%s: %v", string(tables[0]), string(canary), err)
				return false, nil
			}
			klog.Errorf("NOK5G21-145: iptables canary %s/%s deleted", string(tables[0]), string(canary))
			...
			if err != nil {
				klog.Errorf("NOK5G21-145: Inconsistent iptables state detected.")
			}
			return true, nil
		}, stopCh)
		...
		klog.Errorf("NOK5G21-145: Reloading after iptables flush")
		reloadFunc()
	}
}
...
func getIPTablesRestoreWaitFlag(version *utilversion.Version, exec utilexec.Interface, protocol Protocol) []string {
	...
	if err != nil || vstring == "" {
		klog.Errorf("NOK5G21-145: couldn't get iptables-restore version; assuming it doesn't support --wait")
		return nil
	}
	if _, err := utilversion.ParseGeneric(vstring); err != nil {
		klog.Errorf("NOK5G21-145: couldn't parse iptables-restore version; assuming it doesn't support --wait")
		return nil
	}
	return []string{WaitString}
}
